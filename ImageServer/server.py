...
class Server():
    ...
    def handle_client(self, conn, addr):
        ...
        while connected:
            ...
            self.buffer = conn.recv(4000000)  # args: number of bytes we want to receive
            if len(self.buffer) != 0:
                message = Message(self.buffer, addr)
                message.handle_receive()
                if os.path.isdir("ImageReceived"):
                    image = Image.open(io.BytesIO(message.content["image-bytes"]))
                    image_name = message.content["image-name"]
                    image.save(f'ImageReceived/{image_name}')
                else:
                    os.makedirs("ImageReceived")
                    image = Image.open(io.BytesIO(message.content["image-bytes"]))
                    image_name = message.content["image-name"]
                    image.save(f'ImageReceived/{image_name}')
