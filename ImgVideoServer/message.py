...
class Message():
    ...
    def process_request(self):
        ...
        elif self.json_header["content-type"] == "image":
            encoding = self.json_header["content-encoding"]
            self.content = json_decode(data, encoding)
            self.content["image-bytes"] = bytearray(self.content["image-bytes"]) # convert list back to bytes
        else:
            self.content = data
    ...
    def create_image_byte_data(self, image_name: str, content_type="image"):
        # create a json for image encoding and metadata
        image_dict = {
            "image-name": image_name,
            "image-bytes": list(self.buffer) # have to convert bytes to list because json can't serialized bytes
        }
        content_bytes = json_encode(image_dict)
        self.json_header = {
            "byteorder": sys.byteorder,
            "content-length": len(content_bytes),
            "content-type": content_type,
